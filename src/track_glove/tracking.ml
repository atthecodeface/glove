...
module Camera =
  struct
    ...
    let add_point t x y =
      let pt = Vector.make2 (x -. (float t.cx)) ((float t.cy) -. y) in
      t.pts <- t.pts @ [pt]
    ...
    let add_point_vec t xy =
      ...
      add_point t x y
    (*f angles_of_pt t pt:vec2 - get 'pitch/roll' relative to camera *)
    ...
    let line_of_xy t x y = 
      let pr = angles_of_xy t (x -. (float t.cx)) (y -. (float t.cy)) in
      line_of_pr t pr
    let line_of_pt t pt = 
      (*
        Determine direction vector (x,y,z) for the point
        (for no real lens projection should be k*(x,y,1))
       *)
      let pr = angles_of_pt t pt in
      line_of_pr t pr
    let pr_of_xyz t xyz = 
      (* Determine pitch/roll of an xyz from the projection *)
      let xyz = Vector.(add_scaled (-1.0) t.xyz (copy xyz)) in
    ...
    let distance_between_xys cs xyz =
    ...
    let error_of_mapping_in_3d cs (p0,p1) =
      ...
      abs_float d
    let error_of_mapping_in_2d cs (p0,p1) =
      let l0 = line_of_pt cs.(0) (get_point cs.(0) p0) in
      let l1 = line_of_pt cs.(1) (get_point cs.(1) p1) in
      let (midpoint, d) = Line.midpoint_between_lines l0 l1 in
      distance_between_xys cs midpoint
    let error_of_mapping = error_of_mapping_in_2d
    let error_of_mappings cs pt_maps =
      let acc_error_of_mapping so_far (p0,p1) =
        so_far +. (error_of_mapping cs (p0,p1))
      in
      List.fold_left acc_error_of_mapping 0. pt_maps
    let find_min_error cs mappings =
      let find_min_error_step zrot yrot step =
        let result = ref (100000., (0., 0.)) in
        for zr=0 to 20 do
          for yr=0 to 20 do
            let z = zrot +. ((float (zr-10)) *. step) in
            let y = yrot +. ((float (yr-10)) *. step) in
            cs.(1).zrot <- z;
            cs.(1).yrot <- y;
            let err = error_of_mappings cs mappings in
            Printf.printf "   %f z %f y %f\n" err z y;
            if (err<(fst (!result))) then result := (err, (z, y));
          done
        done;
        !result
        in
        (*let (e, (z, y)) = find_min_error_step 0. 0. (pi /. 20.) in*)
        let (e, (z, y)) = find_min_error_step 0. 0.5 (pi /. 60.) in
        Printf.printf "Error %f z %f y %f\n" e z y;
        let (e, (z, y)) = find_min_error_step z y 0.05 in
        Printf.printf "Error %f z %f y %f\n" e z y;
        let (e, (z, y)) = find_min_error_step z y 0.01 in
        Printf.printf "Error %f z %f y %f\n" e z y;
(*        let (e, (z, y)) = find_min_error_step z y 0.003 in
        Printf.printf "Error %f z %f y %f\n" e z y;
        let (e, (z, y)) = find_min_error_step z y 0.001 in
        Printf.printf "Error %f z %f y %f\n" e z y;
        let (e, (z, y)) = find_min_error_step z y 0.0003 in
        Printf.printf "Error %f z %f y %f\n" e z y;
        let (e, (z, y)) = find_min_error_step z y 0.0001 in
        Printf.printf "Error %f z %f y %f\n" e z y;
        let (e, (z, y)) = find_min_error_step z y 0.00003 in
        Printf.printf ">Error %f z %f y %f\n" e z y;
 *)        cs.(1).zrot <- z;
        cs.(1).yrot <- y;
        List.iter (fun (p0,p1) -> Printf.printf "Mapping %d %d err %f\n" p0 p1 (error_of_mapping cs (p0,p1))) mappings;
        (e, (z, y))
  end
