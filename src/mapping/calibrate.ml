...
let data = List.map (fun (x,y) -> Point.make (-.x)  (-.y)) data_c1
let data = List.map (fun (x,y) -> Point.make (-.y)  x) data_c1
let data = List.map (fun (x,y) -> Point.make y  (-.x)) data_c1
let data = List.map (fun (x,y) -> Point.make x y ) data_c0
 ...
 *)
let _ =
  let pt_sets = Point_set.make_from_points 4 0.98 data in
