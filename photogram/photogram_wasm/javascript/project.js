...
class UndoableNpDelete {
    ...
    rev(p) {
        ...
        if (this.np.at_infinity) {
            ...
            p.wasm_project.nps.set_model(this.np_name, this.np.model_as_array(), this.np.uncertainty);
        }
        p.np_changed(false, this.np_name);
    }
}
...
export class Project {
    ...
    set_cip(cip_number, delta = false) {
        var _a;
        if (delta) {
            cip_number += this.cip.cip_number;
        }
        const cip_name = (_a = this.wasm_project) === null || _a === void 0 ? void 0 : _a.cip_name(cip_number);
