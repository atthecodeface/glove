...
export class ZoomedWindow {
  ...
  max_zoom: number = 1.0;
  zoom: number = 1.0;
  img_wh: [number, number] = [0, 0];
  scr_wh: [number, number] = [0, 0];
  zoom_wh: [number, number] = [0, 0];
  ...
  get_scr_wh(): [number, number] {
    return this.scr_wh;
  }

  get_img_cxy(): [number, number] {
    return [this.img_wh[0] / 2, this.img_wh[1] / 2];
  }

  get_zoomed_img_bounds(): [number, number, number, number] {
  ...
  set_img(w: number, h: number): void {
    this.img_wh = [w, h];
    this.recalculate_zoom();
  }

  set_zoom_scr(lx: number, ty: number): void {
    this.zoom_scr_ofs[0] = lx;
    this.zoom_scr_ofs[1] = ty;
  }

  zoom_scr_by(lx: number, ty: number): void {
    this.zoom_scr_ofs[0] += lx;
    this.zoom_scr_ofs[1] += ty;
  ...
  zoom_set(zoom: number, focus_xy?: [number, number]): number {
  ...
  scr_xy_of_img_xy(img_xy: [number, number]): [number, number] {
    return [
  ...
  img_xy_of_scr_xy(scr_xy: [number, number]): [number, number] {
    return [
  ...
  img_bounds(): [number, number, number, number] {
    ...
    const img_rx =
      (this.zoom_scr_ofs[0] + this.zoom_wh[0]) / this.zoom_px_of_img_px -
      img_cxy[0];
    const img_by =
      (this.zoom_scr_ofs[1] + this.zoom_wh[1]) / this.zoom_px_of_img_px -
      img_cxy[1];
  ...
  // possible)
  scr_focus_on_xy(img_xy: [number, number]): void {
