...
enum PointMappingModeKind {
  Normal,
  Stars,
}
class PointMappingMode {
  mode_kind: PointMappingModeKind = PointMappingModeKind.Normal;
  show_stars(): boolean {
  ...
  implements WebglCanvasClient, ToolsDialogClient<number>, MappedNpClient, ProjectClient
{
  ...
  log: Logger;
  mode: PointMappingMode = new PointMappingMode();

  html_div: HtmlElement;
  zoomed_window: ZoomedWindow;
  tools_dialog: ToolsDialog<number>;
  tools_nps_div: HtmlElement;
  tools_cips_div: HtmlElement;
  tools_stats_div: HtmlElement;
  animate: Animate;

  star_catalog: WasmCatalog = new WasmCatalog("hipp_full");
  camera: WasmCameraInstance | null = null;

  wasm_star: WasmStar;
  wasm_mat4: WasmMat4f64;
  wasm_vec2: WasmVec2f64;
  ...
  mm_scr_width: number = 40;
  // mm_scr_width / current width of screen
  mm_per_scr_px : number = 40 / 800.0;
  ...
  tools_dialog_add_tabs(tools_dialog: ToolsDialog<number>, tabs: Tabs<number>) {
    ...
    this.tools_cips_div = tabs.add_tab(
      tools_dialog.add_tab_div("tab-sc-cips", "dialog_inner_contents"),
      "CIPS",
      2,
    );
  ...
  rotate_camera(axis: number, amount: number) {
    if (this.camera === null) { return; }
    this.camera.orientation_set_quat(this.wasm_quat);
    switch (axis) {
      case 1: { this.wasm_quat.set_premul_rotate_y(amount / 180 * 3.1415); break; }
      case 2: { this.wasm_quat.set_premul_rotate_z(amount / 180 * 3.1415); break; }
      default: { this.wasm_quat.set_premul_rotate_x(amount / 180 * 3.1415); break; }
    }
  ...
  adjust_camera_orientation_by_dxy2(angle:number, max_steps:number) {
  ...
  find_star_closest_to_pm(np_name:string, max_angle:number): boolean {
    const wasm_cip = this.application.current_project().get_wasm_cip();
    if (wasm_cip === null) { return false; }
   ...
   */
  selected_star_pts: Float32Array = new Float32Array(0);
  selected_star_pts_epoch: number = 0;
  cached_stars_epoch: number = 0;
   ...
   * scale of grid lines required in both directions
   */
  redraw_interesting_points(webgl: Webgl, webgl_canvas: WebglCanvas, projection_zoom:number): void {
    if (this.camera === null) { return; }
    const m_a = this.application.wasm_memory.float_array_of_mat4f64(
      this.wasm_mat4,
    );
  ...
  uniform_xy_of_img_xy(x: number, y: number): [number, number] {
    if (this.camera === null) { return [x, y]; }
    return [
