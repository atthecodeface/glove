...
static VTABLE: &std::task::RawWakerVTable = &std::task::RawWakerVTable::new(
    |x| std::task::RawWaker::new(x, VTABLE), // clone: unsafe fn(_: *const ()) -> RawWaker,
    |_| (),                                  // wake: unsafe fn(_: *const ()),
    |_| (),                                  // wake_by_ref: unsafe fn(_: *const ()),
    |_| (),                                  // drop: unsafe fn(_: *const ())
);

fn complete<T, F: Future<Output = T>>(mut thing: F) -> T {
    let data = &() as *const ();

    let raw_waker = std::task::RawWaker::new(data, VTABLE);
    let waker = unsafe { std::task::Waker::from_raw(raw_waker) };
    let mut context = std::task::Context::from_waker(&waker);
    let mut adapter = unsafe { Pin::new_unchecked(&mut thing) };
    eprintln!("Waiting for thing");
    loop {
        eprintln!("Poll");
        match adapter.as_mut().poll(&mut context) {
            Ready(val) => {
                eprintln!("Done waiting for thing");
                return val;
            }
            _ => (),
        }
...
fn execute_gpu(numbers: &[u32]) -> Option<Vec<u32>> {
    ...
    let adapter = complete(adapter).unwrap();
    dbg!(&adapter);

    // `request_device` instantiates the feature specific connection to the GPU, defining some parameters,
    //  `features` being the available features.
    let (device, queue) = complete(adapter.request_device(
    ...
    let compute_pipeline = device.create_compute_pipeline(&pipeline_desc);
